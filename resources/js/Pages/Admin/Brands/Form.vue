...
const isEdit = Boolean(props.brand);
...
<template>
    ...
    <AdminLayout>
        <div class="mb-6">
            <Link :href="route('admin.brands.index')" class="text-xs font-semibold text-slate-500 hover:text-brand-600">← Back to Brands</Link>
            <h1 class="mt-1 text-xl font-bold text-navy-900">{{ isEdit ? 'Edit Brand' : 'New Brand' }}</h1>
        </div>
        ...
        <form @submit.prevent="submit" enctype="multipart/form-data" class="max-w-xl space-y-6">
            <section class="space-y-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <TextInput v-model="form.name" label="Name" required :error="form.errors.name" placeholder="e.g. XCMG" />
            ...
            <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <img v-if="isEdit && !form.logo && brand.logo_path" :src="`/storage/${brand.logo_path}`" class="mt-4 h-20 w-40 rounded-xl border border-slate-200 bg-white object-contain p-2" />
            </section>
            ...
            <div class="flex items-center gap-3">
                ...
                <Link :href="route('admin.brands.index')" class="text-sm font-medium text-slate-500 hover:text-slate-700">Cancel</Link>
            </div>
        </form>
    </AdminLayout>
</template>
